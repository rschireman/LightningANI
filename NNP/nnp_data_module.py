...
class NNPDataModule(pl.LightningDataModule):
        def __init__(self, data_dir: str = "./", batch_size: int=32, use_cuda_extension: bool = False):
            ...
            self.training = torch.utils.data.DataLoader(list(self.training), batch_size=batch_size,  num_workers=64, pin_memory=True)
            self.validation = torch.utils.data.DataLoader(list(self.validation), batch_size=batch_size, num_workers=64, pin_memory=True)
